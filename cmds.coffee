...
###
a msg to set in #announcements for 
the cosmicstar network
###
...
exports.announce = (msg, args) ->
    color = title = null
    content = "IP ≫ play.cosmicstarpe.co.uk\nDiscord ≫ https://discord.gg/MxUbwtS\n\n"
    for arg in args
        if arg.startsWith "c::"
            clrstr = arg.substr 3
            color = if clrstr of main.color then main.color[clrstr] else null
...
exports.clear = (msg, args) ->
    ...
    if count > 125
        main.sendEmbed msg.channel,
                       """
                       The maximum amount of messages to purge at once is 125, to limit API exceedance.
                       """,
                       "Limit exceeded", main.color.red
            .then (m) -> setTimeout (-> bot.deleteMessage m.channel.id, m.id), 10000
    else
        yourmsgs = []
        bot.getMessages msg.channel.id, 100
            .then (msgs) ->
                ...
                for m in yourmsgs[1..count]
                    ...
                    catch e then console.log "Failed cleaning message. Missing permissions?"